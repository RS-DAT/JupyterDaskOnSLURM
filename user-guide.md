...
Advantages of the former approach include the possibility to easily automate the process of building the environment image (the Tykky container wrapper requires one to build the image on the cluster where it will run). Advantages of using the Tykky container wrapper include the availability of tools to update the environment without having to rebuild the container image.
